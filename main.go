                ...
                for d := start; d.Before(end); d = d.AddDate(0, 0, 1) {
                    ...
                    if dataType == "orderbooks" {
                        for hour := 0; hour < 24; hour++ {
                            ...
                            go func(hour int, dataType string) {
                                hourStr := fmt.Sprintf("%02d", hour)
                                url := fmt.Sprintf("%s%s/%s/%d%s/%s-%d%s%s%s.csv.gz", baseurl, biz, dataType, year, month, market, year, month, day, hourStr)
                                filepath := filepath.Join(dataTypeDir, fmt.Sprintf("%s-%s-%d%s%s%s.csv.gz", market, dataType, year, month, day, hourStr))
